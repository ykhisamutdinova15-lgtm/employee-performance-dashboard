...
|Targets|72|Содержит целевые значения KPI для каждого КЦ и отчетного периода. <br />Для каждого сочетания «контактный центр + период» хранится отдельная запись по каждому KPI.|
