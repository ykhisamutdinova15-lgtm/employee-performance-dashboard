...
|-|-|-|
|[Results.csv](./data/Results.csv)|600|Содержит фактические результаты сотрудников по четырем KPI за каждый отчетный месяц. <br />Одна запись соответствует одному сотруднику за один отчетный период.|
|[Targets.csv](./data/Targets.csv)|72|Содержит целевые значения KPI для каждого КЦ и отчетного периода. <br />Для каждого сочетания «контактный центр + период» хранится отдельная запись по каждому KPI.|
...
Для автоматизации оценки эффективности сотрудников был разработан единый SQL-скрипт, реализующий полный цикл подготовки данных для последующей визуализации в Power BI.<br />
[Посмотреть полный SQL-скрипт](./sql/employee_performance.sql)<br />
...
* **Common Table Expressions (CTE)** — последовательное построение этапов обработки данных;
* **JOIN** — объединение фактических результатов с таргетами KPI;
* **CASE** — реализация условий выполнения KPI, динамики и правил классификации;
* **Оконные функции (Window Functions)** — определение хронологии и работа со скользящим периодом;
* **Агрегатные функции** — расчет сводных показателей выполнения KPI и динамики;
* **WHERE / HAVING** — фильтрация данных на различных этапах расчета.

**Структура SQL-скрипта в разрезе CTE:**
...
8. **final\_calc** - расчет количества выполненных показателей, показателей с динамикой/ без динамики;
9. **Final Select** - формирование аналитической витрины для Power BI с применением правил классификации.
    
Архитектура SQL построена по принципу последовательной обработки данных: каждый CTE отвечает за отдельный этап расчета и формирует основу для следующего шага алгоритма.
