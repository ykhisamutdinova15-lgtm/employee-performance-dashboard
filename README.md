...
**Примечание:** при первоначальном импорте `Results.csv` в использованной SQLite-среде столбец `Group` был автоматически импортирован под именем `col2`. В SQL-скрипте это техническое имя используется на нескольких этапах обработки. При использовании другой среды импорта, где столбец будет корректно распознан как `Group`, необходимо заменить `col2` на `Group` в соответствующих местах SQL-скрипта.
